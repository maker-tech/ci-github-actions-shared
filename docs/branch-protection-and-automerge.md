...
This guide covers the complete setup for protecting the `dev` branch with GitHub Rulesets and enabling Renovate to automerge low-risk dependency updates without human review.

## What this achieves

- Human PRs require **2 approving reviews** and **passing CI checks** before merging to `dev`.
- Renovate dependency PRs (patch and minor) **automerge without human review** once CI checks pass.
...
- The Renovate GitHub App installed on the repo/org.
- The `quality-checks.yml` and `lint-pr-title.yml` workflows committed and run at least once (so check names appear in GitHub's dropdown).

---

## Step 1 — Remove classic branch protection on `dev`
...
The `CI Gate` job in `.github/workflows/quality-checks.yml` solves a specific problem with required status checks and conditional CI.
...
This is the full `quality-checks.yml` for reference:
...
The Renovate config at `.github/renovate.json` controls which dependency updates automerge.
